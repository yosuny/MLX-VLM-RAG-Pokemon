...
# RAG Engine might fail if model not loaded, but we only init it in Pass 1
...
def prepare_dataset(output_path="data/eval_v3.json"):
    print("Selecting 10 random samples (5 Train, 5 Valid)...")
...
def evaluate_v3():
    dataset_path = "data/eval_v3.json"
    ...
    # Generic Prompt (No "Pokemon" hint)
    neutral_prompt = "What is this? Answer in English and Korean."
    
    # -------------------------------------------------------------
    # PASS 1: VANILLA + RAG (Base Model)
    # -------------------------------------------------------------
    print("\n" + "="*80)
    ...
    for i, item in enumerate(dataset):
        print(f"[{i+1}/10] {item['split'].upper()}: {item['image_path']}")
        image_path = item['image_path']
        
        # 1. Vanilla
        prompt_vanilla = apply_chat_template(processor, config=model.config, prompt=neutral_prompt, num_images=1)
        ...
        prompt_rag = apply_chat_template(processor, config=model.config, prompt=neutral_prompt + rag_context, num_images=1)
    ...
    # Fix tokenizer regex warning if needed
    if hasattr(processor, "image_processor"):
        processor.image_processor = RobustImageProcessorWrapper(processor.image_processor)
    ...
    for i, item in enumerate(dataset):
        ...
        prompt_tuned = apply_chat_template(processor, config=model.config, prompt=neutral_prompt, num_images=1)
    ...
    # Generate Report
    report_path = "EVALUATION_REPORT_v3.md"
    with open(report_path, "w") as f:
        f.write("# Evaluation Report v3: Final Model Comparison\n\n")
        f.write(f"**Prompt**: `{neutral_prompt}`\n")
